...
public class ValidateUserEvent extends BaseHandler {
    ...
    private void handleCreateReservationAirline(String json) {
        EventData event = EventData.fromJson(json, AgencyReservationAirlineCommmad.class);
        AgencyReservationAirlineCommmad command = (AgencyReservationAirlineCommmad) event.getData();
        UserDTO user = this.userService.getUserByEmail(command.getCreateReservationCommand().getCustomerInfo().getEmail());
        if (user == null) 
            return;
